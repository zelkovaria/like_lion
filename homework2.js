...
const todoControl = (command, value) => {
  ...
  switch(command){
    case 'create':
      createTodo(value);
      //해당 함수에 새로 추가될 todo의 정보를 인자로 받아야함 -> 'id', 'title', 'text'로 구성된 객체로 표현됨
      //command는 어떤 명령어를 실행할 지를 지정하는 인자이므로 value 값이 들어가야함
      break;
    case 'read':
      readTodo(value);
      break;
...
const readTodo = () => {
  ...
  if(todos.length ===0) {
    console.log('id = ${todo.id}에 맞는 todo를 찾을 수 없습니다.');
  }
...
const updateTodo = (targetTodo) => {
  ...
  /*
  const { id, title, text } = targetTodo;
  const foundIndex = todos.findIndex((todo) => todo.id === id);
  if (foundIndex === -1) {
    console.log(`id = ${id}에 맞는 todo를 찾을 수 없습니다.`);
    return;
  }
  todos[foundIndex] = { id, title, text };
  */
};
...
const test = () => {
  //todoControl함수가 잘 작동하는지 확인하는 코드들
  todoControl('bug!!!');
 ...
 // map()함수는 배열의 각 요소에 대해 지정된 함수를 호출하고, 그 결과로 새로운 배열을 반환함 
